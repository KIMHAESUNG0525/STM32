...
unsigned int elevator_bell[] =
 {
 	C5, E5
 };

 void ELEVATOR_BUZZER() //내부 ELE 타겟 층 도착했을 때 부저
 {
 	int divide_freq = 1600000; // 4KHZ 부저 주파수를 내기 위해 기본 클럭을 분주해서 얻은 주파수

 	HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1) ;

 	//TIM1->CCER |= TIM_CCER_CC1E;   // 채널 1 출력 (CC1E 비트 설정) Page 302 ~ 304 참조
 	//TIM1->CR1 |= TIM_CR1_CEN;      // 타이머 시작 (CEN 비트 설정), TIMx_CR1 0번 레지스터 활성화
     for (int i=0; i < 2; i++)
     {
 		__HAL_TIM_SET_AUTORELOAD(&htim1, divide_freq / elevator_bell[i]);
 		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, divide_freq / elevator_bell[i] / 2);  // Duty를 50%로 설정
 		HAL_Delay(200);
 		noTone();
     }
...
void set_buzzer(int frequency)
{
	...
	//TIM1 ->CCR4 = divide_freq / 2000 / 2; // 수정필요
