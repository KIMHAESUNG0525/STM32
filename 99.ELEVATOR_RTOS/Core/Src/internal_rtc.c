...
extern ElevatorState current_state;
...
//10.18 mutex ---------------------------------------
void get_rtc_date(void) //Text LCD date function / Mutex 활용
{
	static RTC_TimeTypeDef oldTime = {0}; // 시각 정보 //쓰레기값 방지를 위해 0으로 초기화 , 지역변수지만 전역변수처럼 사용
...
//10.18 mutex ---------------------------------------
void get_rtc_time(void) // Text LCD time function // Mutex 활용
{
	...
	if (oldTime.Seconds != sTime.Seconds)
	{
			...
			{
				strcpy(sfloor, "     1F"); //내,외부 ELE 1층 LCD
			}
			else if (current_floor == floor_2)
			{
				strcpy(sfloor, "     2F"); //내,외부 ELE 2층 LCD
			}
			else if (current_floor == floor_3)
			{
				strcpy(sfloor, "     3F"); //내,외부 ELE 3층 LCD
			}
			else if (current_floor == floor_4)
			{
				strcpy(sfloor, "     4F"); //내,외부 ELE 4층 LCD
			}

			switch (current_state)
			{
				case ELEVATOR_IDLE:
					sprintf(scm,"%s", sfloor); //내,외부 ELE 현재 위치층 LCD
					break;
				case ELEVATOR_FORWARD:
					sprintf(scm,"     UP"); //내,외부 ELE UP LCD
					break;
				case ELEVATOR_BACKWARD:
					sprintf(scm,"   DOWN"); //내,외부 ELE DOWN LCD
					break;
				move_cursor(1, 18);
			}
			lcd_string(scm);
