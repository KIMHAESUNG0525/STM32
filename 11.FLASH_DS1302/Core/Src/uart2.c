...
extern void set_alarm_time(char *time); //10.23
extern void ds1302_set_time(char *time); //10.24
extern void ds1302_time_flash_read(void); //10.24
...
void pc_command_processing(void)
{
	...
	if(front != rear) //data가 rx_buff에 존재 한다.
	{
		...
		if(strncmp(rx_buff[front], "led_all_on",strlen("led_all_on")) == 0 )
			...
			dot_on = 0;
		else if (strncmp(rx_buff[front], "set_alarm_time",strlen("set_alarm_time")) == 0 ) //10.23
			set_alarm_time(&rx_buff[front][strlen("set_alarm_time")]);
		else if (strncmp(rx_buff[front], "ds1302_set_time",strlen("ds1302_set_time")) == 0 ) //10.24
			ds1302_set_time(&rx_buff[front][strlen("ds1302_set_time")]);
		else if (strncmp(rx_buff[front], "ds1302_time_flash_read",strlen("ds1302_time_flash_read")) == 0 ) //10.24
			ds1302_time_flash_read();
		front++;
